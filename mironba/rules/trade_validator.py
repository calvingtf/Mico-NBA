...
def _check_team(
    ...
    for candidate in incoming:
        bucket = (
            minimum_players
            if qualifies_for_minimum_exception(
                trade.season,
                candidate.salary,
    ...
    usable_tpes = _usable_trade_exceptions(state, trade.season, tier_after)
...
def _usable_trade_exceptions(
    ...
    second-apron teams may not use one generated by a sign-and-trade.
    """
    usable = []
    for tpe in state.trade_exceptions:
        if tier_after >= ApronTier.FIRST_APRON and tpe.created_season != season:
            continue
        if tier_after >= ApronTier.SECOND_APRON and tpe.from_sign_and_trade:
            continue
        usable.append(tpe)
    return usable
...
def _check_aggregation(
    ...
    if needs_aggregation and tier_after >= ApronTier.SECOND_APRON:
        findings.append(
            Finding(
...
def _check_cash(
    ...
    for state in trade.teams:
        ...
        if sent and tier_after >= ApronTier.SECOND_APRON:
            findings.append(
                Finding(
